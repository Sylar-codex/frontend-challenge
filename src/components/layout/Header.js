...
import chevronRight from "../../assets/icons/chevron-right-70.svg";
    ...
    >
      ...
      <div className="flex lg:flex-row flex-col-reverse lg:items-center lg:space-x-6 mt-2 lg:mt-0">
        ...
        <div className="flex lg:flex-row flex-row-reverse lg:justify-normal justify-between items-center lg:space-x-5 mt-2 lg:mt-0">
          ...
          {/* profile info div */}
          <div className="relative lg:w-64 w-20">
            <div
              onClick={() => {
                setDropDown((prev) => !prev);
              }}
              className="flex hover:cursor-pointer p-2 w-full h-14 rounded-full space-x-2 lg:space-x-0 lg:border-2 lg:border-solid lg:border-headerStroke lg:justify-between justify-center items-center"
            >
            ...
            {dropDown && (
              <GoToProfile darkMode={darkMode} setDropDown={setDropDown} />
            )}
          </div>
        </div>
      </div>
...
function GoToProfile({ darkMode, setDropDown }) {
  return (
    <div
      onClick={() => {
        setDropDown(false);
      }}
      className={`absolute top-16 hover:cursor-pointer left-0 rounded-xl border-headerStroke lg:text-base text-sm shadow-lg w-64 ${
        darkMode ? "bg-lighter-grey" : "bg-white"
      } z-10`}
    >
      <div className="flex items-center justify-between p-2">
        <div className="rounded-full w-12 h-12">
          <img className="w-full h-full" src={headerProfilePic} />
        </div>
        <div>
          <h3>Justin Bergson</h3>
          <p className="text-sm text-[#787486]">Justin@gmail.com</p>
          <p className="text-primary text-sm">View Profile</p>
        </div>
        <div className="w-5 h-5">
          <img className="w-full h-full" src={chevronRight} alt="" />
        </div>
      </div>
    </div>
  );
